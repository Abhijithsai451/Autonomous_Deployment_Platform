...
class AgentRegistry:
    ...
    def resolve(self, slug: str)-> BaseAgent:
        """
        Loads Agent Entity from the database by slug, checks if ACTIVE, and
        Instantiates its executable python class
        """
        agent_record = self.agent_repo.get_by_slug(slug)
        if not agent_record:
