...
async def start_task(id: UUID, payload: Optional[StartTaskSchema] = None, db: Session = Depends(workflow_db_session)):
    svc = TaskService(db)
    assigned_agent_id = payload.assigned_agent_id if payload else None
    try:
        return await svc.start_task(id, assigned_agent_id=assigned_agent_id)
...
async def fail_task(id: UUID, payload: Optional[FailTaskSchema] = None, db: Session = Depends(workflow_db_session)):
    svc = TaskService(db)
    error_details = payload.error_details if payload else {}
    try:
        return await svc.fail_task(id, error_details=error_details)
    except InvalidStateTransitionError as e:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=str(e))
