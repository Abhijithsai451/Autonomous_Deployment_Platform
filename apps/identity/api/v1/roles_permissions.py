...
@router.post("/roles")
async def create_role(payload: CreateRoleSchema, db: Session = Depends(get_db_session)):
...
def patch_role(id: UUID, payload: dict, db: Session = Depends(get_db_session)):
...
def delete_role(id: UUID, db: Session = Depends(get_db_session)):
    role = db.query(Role).filter(Role.id == id).first()
    if not role: raise HTTPException(status_code=404, detail="Role not found")
    db.delete(role)
    db.commit()
...
def get_permissions(db: Session = Depends(get_db_session)):
