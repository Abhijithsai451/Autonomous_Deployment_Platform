...
@router.post("")
async def create_key(payload: CreateKeySchema, db: Session = Depends(get_db_session)):
...
@router.get("")
def list_keys(db: Session = Depends(get_db_session)):
    return db.query(ApiKey).filter(ApiKey.revoked_at.is_(None)).all()

@router.delete("/{id}")
async def revoke_key(id: UUID, db: Session = Depends(get_db_session)):
    svc = IdentityService(db, KeycloakClient())
