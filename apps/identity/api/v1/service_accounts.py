...
@router.post("")
async def create_sa(payload: CreateSASchema, db: Session = Depends(get_db_session)):
...
@router.get("")
def list_sa(db: Session = Depends(get_db_session)):
    return db.query(ServiceAccount).all()

@router.patch("/{id}")
def patch_sa(id: UUID, payload: dict, db: Session = Depends(get_db_session)):
...
@router.delete("/{id}")
def delete_sa(id: UUID, db: Session = Depends(get_db_session)):
    sa = db.query(ServiceAccount).filter(ServiceAccount.id == id).first()
    if not sa: raise HTTPException(status_code=404, detail="Service Account not found")
    db.delete(sa)
    db.commit()
