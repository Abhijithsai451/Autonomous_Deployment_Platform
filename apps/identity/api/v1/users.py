...
@router.get("/{id}")
def get_user(id: UUID, db: Session = Depends(identity_db_session)):
    user = db.query(User).filter(User.id == id).first()
    if not user: raise HTTPException(status_code=404, detail="User not found")
    return user

@router.patch("/{id}")
def patch_user(id: UUID, payload: dict, db: Session = Depends(identity_db_session)):
    user = db.query(User).filter(User.id == id).first()
    if not user: raise HTTPException(status_code=404, detail="User not found")
...
async def change_status(id: UUID, payload: StatusUpdateSchema, db: Session = Depends(identity_db_session)):
    svc = IdentityService(db, KeycloakClient())
    return await svc.update_user_status(id, payload.status)

@router.delete("/{id}")
async def delete_user(id: UUID, db: Session = Depends(identity_db_session)):
    svc = IdentityService(db, KeycloakClient())
    await svc.delete_user(id)
    return {"status": "deleted"}

@router.post("/{id}/roles")
async def assign_role(id: UUID, role_id: UUID, db: Session = Depends(identity_db_session)):
    svc = IdentityService(db, KeycloakClient())
    await svc.assign_role(id, role_id)
    return {"status": "assigned"}

@router.delete("/{id}/roles/{roleId}")
async def remove_role(id: UUID, roleId: UUID, db: Session = Depends(identity_db_session)):
    svc = IdentityService(db, KeycloakClient())
